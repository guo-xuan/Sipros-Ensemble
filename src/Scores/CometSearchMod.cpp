 ...
 * vdMZ
 */
bool CometSearchMod::PreprocessVector(struct Query *pScoring, MS2Scan * mstSpectrum, vector<double> & pdTmpRawData, vector<double> & pdTmpFastXcorrData,
...
//  Reads MSMS data file as ASCII mass/intensity pairs.
bool CometSearchMod::LoadIonsVector(struct Query *pScoring, vector<double> & pdTmpRawData, MS2Scan * mstSpectrum, struct PreprocessStruct *pPre) {
	double dIon, dIntensity;
	double dInverseBinWidth = mstSpectrum->isMS2HighRes ? (ProNovoConfig::dHighResInverseBinWidth) : (ProNovoConfig::dLowResInverseBinWidth);
	double dOneMinusBinOffset = mstSpectrum->isMS2HighRes ? (ProNovoConfig::dHighResOneMinusBinOffset) : (ProNovoConfig::dLowResOneMinusBinOffset);
	size_t i = 0;
...
// pdTmpRawData now holds raw data, pdTmpCorrelationData is windowed data after this function
void CometSearchMod::MakeCorrDataVector(vector<double> & pdTmpRawData, vector<double> & pdTmpCorrelationData, struct Query *pScoring,
		struct PreprocessStruct *pPre) {
	int i, ii, iBin, iWindowSize, iNumWindows = 10;
	double dMaxWindowInten, dTmp1, dTmp2;
...
bool CometSearchMod::ScorePeptidesSIP(vector<vector<double> > & vvdYionMass, vector<vector<double> > & vvdYionProb, vector<vector<double> > & vvdBionMass,
